...
class HomeView (UserControl):
    ...
    def build (self):
        ...
        #main column where each app component will be placed and displayed
        return Column( controls=[
                # class instances go here...
                AppNavbar(),
                Container(content=SearchInput(), margin=margin.symmetric(horizontal=20, vertical=15)),
                ElevatedButton("Agregar Libro", icon=icons.ADD, on_click= lambda _: self.page.go("/books/add-book")),
                Container(content=Column(controls=[BooksTable()]), margin=margin.symmetric(horizontal=20, vertical=15)),        
            ])